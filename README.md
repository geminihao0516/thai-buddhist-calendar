...
- 提供標準、大、特大三段字級；所有文字會等比例放大，選擇會保存在裝置上，並有中泰文無障礙名稱與狀態朗讀。
- 採 mobile-first RWD 使用方式：手機為單欄流程，年月日改為較寬的兩列輸入；主要觸控區至少 44px，特大字體在 320px 寬仍可重排而不橫向捲動。
- 複製結果可直接貼給朋友。剪貼簿權限不允許時提供手動選取文字。
- 日期不送往伺服器，也不保存在網址或儲存空間。`localStorage` 只保存語言與字級偏好；無追蹤碼、外部字型或第三方執行期請求。
